...
> With root privileges Windows Subsystem for Linux (WSL) allows users to create a bind shell on any port (no elevation needed). Don't know the root password? No problem just set the default user to root W/ <distro>.exe --default-user root. Now start your bind shell or reverse.</distro>
...
- [MS16-014](https://github.com/SecWiki/windows-kernel-exploits/tree/master/MS16-014) 　\[K3134228\]　　\[remote code execution\]　　(2008/Vista/7)
    ...
- [MS03-026](/tmp/.mount_JoplinqYNWF1/resources/app.asar/MS03-026 "./MS03-026") 　\[KB823980\]　　 \[Buffer Overrun In RPC Interface\]　　(/NT/2000/XP/2003)
...
| `SeBackup` | **Threat** | ***Built-in commands*** | Read sensitve files with `robocopy /b` | \- May be more interesting if you can read %WINDIR%\\MEMORY.DMP<br>\- `SeBackupPrivilege` (and robocopy) is not helpful when it comes to open files.<br>\- Robocopy requires both SeBackup and SeRestore to work with /b parameter. |
...
| `SeTcb` | ***Admin*** | 3rd party tool | Manipulate tokens to have local admin rights included. May require SeImpersonate.<br>To be verified. |     |
